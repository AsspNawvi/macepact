...
public class MaceAbilityListener implements Listener {
    ...
    public void onRightClick(PlayerInteractEvent event) {
        ...
        if (type == MaceType.GOD) return; // handled in GodMaceGUIListener
        ...
        switch (type) {
            ...
            case BLOOD  -> {} // Blood berserk triggers on 3-hit combo
            case SPEED  -> abilitySpeed(p);
            default -> {}
        }
    }

    // MAGMA — shoot 1 fireball (2 charges, recharge 15s each)
    ...
    private void abilityEcho(Player p) {
        ...
        if (result != null && result.getHitEntity() instanceof LivingEntity target) {
            double damage = Math.min(target.getHealth(), 8.0);
    ...
    // SOUL — spawn random hostile mob (30s CD)
    ...
    // VOID — dragon breath projectile
    // 2/3 charges in overworld, 3/3 in the End; vortex smash only in End
    private void abilityVoid(Player p) {
        ...
        // Cap available charges based on dimension
    ...
    // NATURE — vine grapple that pulls players toward you (30s CD)
    private void abilityNature(Player p) {
        ...
        if (result != null && result.getHitEntity() instanceof LivingEntity hitEntity) {
            // Pull the target toward the attacker
    ...
    // FROST — ice ray with cooldown (15s CD)
